...
def create_sidebar(
    ...
    with (
        ui.column()
        .classes("p-4 themed-element overflow-y-auto")
        .style(
            "width: 320px; "
        ...
        with ui.row().classes("items-center gap-2 mb-4"):
            ...
            ui.label("Example Questions").classes("text-lg font-bold").style(
                "color: var(--neo4j-text-primary)"
            )
        ...
        for question in settings.ui.example_questions:
            ...
            ui.button(display_text, on_click=lambda q=question: on_example_click(q)).props(
                "flat align=left"
            ).classes("w-full text-left mb-2 normal-case example-question-btn").style(
                "text-transform: none; "
        ...
        with ui.row().classes("w-full justify-center mt-3"):
            ui.button("RESET CHAT", icon="refresh", on_click=on_reset_click).props("dense").style(
                "background: #014063 !important; color: white !important; border-radius: 6px;"
            )
        ...
        with ui.row().classes("items-center gap-2 mb-2"):
            ...
            ui.label("System Information").classes("text-lg font-bold").style(
                "color: var(--neo4j-text-primary)"
            )

        # LLM Configuration subsection (key-value grid layout)
        ui.label("LLM Configuration").classes("text-base font-semibold ml-6 mb-1").style(
            "color: var(--neo4j-text-primary)"
        )
        ...
        with ui.grid(columns=2).classes("ml-8 gap-x-4 gap-y-1 text-sm"):
            ...
            # Temperature row
            ui.label("Temperature").classes("font-semibold").style(
                "color: var(--neo4j-text-primary)"
            )
        ...
        ui.label("Neo4j Database").classes("text-base font-semibold ml-6 mb-1").style(
            "color: var(--neo4j-text-primary)"
        )
